...
class _AllPropCaptureState extends State<AllPropCapture> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          print(
              'State updated, properties stored: ${_allPropertyResults.length}');
        });
      }
  ...
  Future<void> fetchAllPreviousMonthReadings() async {
    try {
      ...
      String prevMonth = DateFormat.MMMM().format(
          DateTime.now().subtract(Duration(days: 30))); // Example: February
  ...
  Future<void> municipalFetchAndOpenStatementByType({
    required BuildContext context,
    required String type,
    required String cellNumber,
    required String accountNumber, // This can now be water or electricity account number
    required String address,
    required String selectedMonth,
  }) async {
    Fluttertoast.showToast(
      msg: "Opening $type statement for $selectedMonth...",
    );

    final formattedAddress = address.trim();
    final storageRef = FirebaseStorage.instance
        .ref()
        .child("pdfs/$selectedMonth/$cellNumber/$formattedAddress");
    ...
    try {
      ...
      for (var item in listResult.items) {
        // Just check for raw account number match — no prefixing
        if (item.name.contains(accountNumber)) {
  ...
  Widget propertyCard() {
    ...
    print("Rendering ${_allPropertyResults.length} properties");
          ...
          child: Scrollbar(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                final munId = _allPropertyResults[index]['municipalityId'];
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        if (showWater) ...[
                          Text(
                            'Water Account Number: ${_allPropertyResults[index]['accountNumber']}',
                        ...
                        if (showElectricity) ...[
                          Text(
                            'Electricity Account Number: ${_allPropertyResults[index]['electricityAccountNumber']}',
                            style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w400),
                          ),
                        ],
                        ...
                        if (showElectricity) ...[
                          const SizedBox(height: 10),
                          Text(
                            'Electricity Meter Number: ${_allPropertyResults[index]['meter_number'] ?? "N/A"}',
                            style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w400),
                          ),
                          const SizedBox(height: 5),
                          Text(
                            'Previous Month ($previousMonth) Electricity Reading: ${previousMonthElectricReadings[_allPropertyResults[index]['address']] ?? "N/A"}',
                            style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w400),
                          ),
                          const SizedBox(height: 5),
                          Text(
                            'Current Month Electricity Reading for $currentMonth: ${currentMonthElectricReadings[_allPropertyResults[index]['address']] ?? "N/A"}',
                            style: const TextStyle(
                                fontSize: 16, fontWeight: FontWeight.w400),
                          ),
                        ],
                            ...
                            onPress: () async {
                              try {
                                ...
                                String? meterNumber = _allPropertyResults[index]
                                    ['water_meter_number'];
                                String? eMeterNumber =
                                    _allPropertyResults[index]['meter_number'];
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                BasicIconButtonGrey(
                                  onPress: () async {
                                    try {
                                      ...
                                      final propData = _allPropertyResults[index].data() as Map<String, dynamic>;
                                ...
                                BasicIconButtonGrey(
                                  onPress: () async {
                                    _onSubmit(); // Optional loading UI

                                    // 🔍 Extract property-specific details
                                    var property = _allPropertyResults[index];
                                    String waterAcc = property['accountNumber'] ?? '';
                                    String electricAcc = property['electricityAccountNumber'] ?? '';
                                    String propertyAddress = property['address'] ?? '';
                                    String userCellNumber = property['cellNumber'] ?? '';
                                    String munId = property['municipalityId'] ?? '';

                                    final utilTypes = municipalityUtilityMap[munId] ?? [];
                                    final bool handlesWater = utilTypes.contains('water');
                                    final bool handlesElectricity = utilTypes.contains('electricity');

                                    if ((handlesWater && waterAcc.isEmpty) ||
                                        (handlesElectricity && electricAcc.isEmpty) ||
                                        propertyAddress.isEmpty ||
                                        userCellNumber.isEmpty) {
                                      Fluttertoast.showToast(msg: "Error: Missing property details for statement.");
                                      return;
                                    }
                                    ...
                                    // 🔁 Show type selector if both utilities are available
                                    if (handlesWater && handlesElectricity) {
                                          ...
                                          actions: [
                                            ElevatedButton.icon(
                                              onPressed: () {
                                                ...
                                                municipalFetchAndOpenStatementByType(
                                                  context: context,
                                                  type: "water",
                                                  cellNumber: userCellNumber,
                                                  accountNumber: waterAcc,
                                                  address: propertyAddress,
                                      ...
                                      Fluttertoast.showToast(msg: "⚠️ No invoice available for this property.");
                                    }
                                  },
                                ...
                                children: [
                                  Stack(
                                    children: [
                                      BasicIconButtonGrey(
                                        onPress: () async {
                                          final accountNumberPDF =
                                              _allPropertyResults[index]
                                                  ['accountNumber'];
                                          final propertyAddress =
                                              _allPropertyResults[index]
                                                  ['address'];
                                          final userCellNumber =
                                              _allPropertyResults[index]
                                                  ['cellNumber'];
                                          final munId =
                                              _allPropertyResults[index]
                                                  ['municipalityId'];
                                          final utilTypes =
                                              municipalityUtilityMap[munId] ??
                                                  [];

                                          final bool showWater =
                                              utilTypes.contains('water');
                                          final bool showElectricity =
                                              utilTypes.contains('electricity');
                                          final previousMonth = DateFormat(
                                                  'MMMM')
                                              .format(DateTime.now().subtract(
                                                  const Duration(days: 30)));

                                          if (accountNumberPDF == null ||
                                              propertyAddress == null ||
                                              userCellNumber == null) {
                                            Fluttertoast.showToast(
                                                msg:
                                                    "⚠️ Missing property details.");
                                            return;
                                          }

                                          _onSubmit(); // Start loading spinner

                                          if (showWater && showElectricity) {
                                            showDialog(
                                              context: context,
                                              builder: (context) => AlertDialog(
                                                title: const Column(
                                                ...
                                                content: const Text(
                                                    "Choose which statement to view."),
                                                actions: [
                                                  ElevatedButton.icon(
                                                    onPressed: () {
                                                      Navigator.pop(context);
                                            ...
                                            municipalFetchAndOpenStatementByType(
                                              ...
                                              type: "water",
                                              cellNumber: userCellNumber,
                                              accountNumber: accountNumberPDF,
                                              address: propertyAddress,
                                              selectedMonth: previousMonth,
                                            );
