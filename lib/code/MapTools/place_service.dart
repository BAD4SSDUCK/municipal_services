...
class PlaceApiProvider {
  ...
  PlaceApiProvider(this.sessionToken, {http.Client? client})
      : client = client ?? http.Client();

  /// Autocomplete disabled everywhere (no Places API).
  Future<List<Suggestion>> fetchSuggestions(String input, String lang) async {
    return []; // return empty list instead of calling Google Places
  }

  /// Optional helper used elsewhere – also disabled.
  static Future<List<Suggestion>> getSuggestions(String query) async {
    return []; // no-op
  }

  /// Place details disabled – return an empty shell so callers don’t crash.
  Future<Place> getPlaceDetailFromId(String placeId) async {
    return Place(
      streetNumber: '',
      street: '',
